...
{
    private \Src\V2\Cronograma\Infrastructure\FindPdfByIdController $controller;
    private \Src\V2\ClienteConfiguracion\Infrastructure\FindByIdController $controllerConfiguracion;

    public function __construct(
        \Src\V2\Cronograma\Infrastructure\FindPdfByIdController $controller,
    ...
    public function __invoke(Request $request)
    {
        try {
            ...
            $Cronograma = $this->controller->__invoke($request);
            $formatter = new NumeroALetras();

            $fechaRegistro = new \DateTime($Cronograma->getFechaRegistro()->value());

            // Obtener datos de la empresa
            $configuracion = $this->controllerConfiguracion->__invoke($request);

            $pdf = PDF::loadView('comprobantes.ticket-interno-Cronograma', compact('Cronograma', 'configuracion', 'formatter', 'fechaRegistro'))
