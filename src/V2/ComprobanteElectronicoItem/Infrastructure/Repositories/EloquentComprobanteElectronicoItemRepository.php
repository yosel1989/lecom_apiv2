...
namespace Src\V2\ComprobanteElectronicoItem\Infrastructure\Repositories;
...
final class EloquentComprobanteElectronicoItemRepository implements ComprobanteElectronicoItemRepositoryContract
{
    private EloquentModelComprobanteElectronicoItem $eloquentModel;

    public function __construct()
    {
        $this->eloquentModel = new EloquentModelComprobanteElectronicoItem;
    }
    ...
    ): ComprobanteElectronicoItem
    {
        $this->eloquentModel->create([
            'id_comprobante' => $comprobante->getId()->value(),
            'id_cliente' => $comprobante->getIdCliente()->value(),

            'id_unidad_medida' => \App\Enums\EnumUnidadMedida::Servicio->value,
            'codigo' => $boleto->getId()->value(),
            'descripcion' => 'BOLETO INTERPROVINCIAL | RUTA:' . $boleto->getRuta()->value() . ' | DESTINO:' . $boleto->getParaderoDestino()->value(),
            'cantidad' => 1,
            'valor_unitario' => $boleto->getPrecio()->value(),
            'precio_unitario' => $boleto->getPrecio()->value(),
            'descuento' => 0,
            'sub_total' => $boleto->getPrecio()->value(),
        ...
        $items = $this->eloquentModel->where('id_comprobante', $comprobante->getId()->value());
        if($items->count() === 0){
            throw new \InvalidArgumentException('Ocurrio un error al registrar el detalle del comprobante');
        }

        $item = $items[0];
        $output = new ComprobanteElectronicoItem(
            new Id($item->id, false, 'El id del detalle no tiene el formato correcto'),
            ...
            new NumericInteger($item->id_unidad_medida),
            new Id($item->codigo, false, 'El id del código del detalle no tiene el formato correcto'),
            ...
            new ValueBoolean($item->anticipo_regulariza),
            ...
            new Id($item->id_usu_modifico, false, 'El id del usuario que modifico no tiene el formato correcto'),
    ...
    ): ComprobanteElectronicoItem
    {
        ...
        return new ComprobanteElectronicoItem(
