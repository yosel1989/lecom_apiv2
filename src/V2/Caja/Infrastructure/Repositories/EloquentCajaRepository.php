...
final class EloquentCajaRepository implements CajaRepositoryContract
{
    ...
    {
        $models = $this->eloquentModelCaja->with('sede:id,nombre')->where('idCliente',$idCliente->value())->get();
    ...
    {
        $model = $this->eloquentModelCaja->with('sede:id,nombre')->findOrFail($idCaja->value());
        $OModel = new Caja(
            new Id($model->id , false, 'El id del caja no tiene el formato correcto'),
