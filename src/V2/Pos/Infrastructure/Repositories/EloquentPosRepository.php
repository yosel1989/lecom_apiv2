...
final class EloquentPosRepository implements PosRepositoryContract
{
    ...
    {
        $models = $this->eloquentModelPos->with('sede:id,nombre')->where('idCliente',$idCliente->value())->get();
    ...
    {
        $model = $this->eloquentModelPos->with('sede:id,nombre')->findOrFail($idPos->value());
        $OModel = new Pos(
            new Id($model->id , false, 'El id del pos no tiene el formato correcto'),
